...
void DynamicVoronoi3D_float(float3 Coordinates, float Time, float AngleOffset, float CellDensity, out float Out, out float Cells) {

    float4 UV = float4(Coordinates, Time);
    float4 g = floor(UV * CellDensity);
    float4 f = frac(UV * CellDensity);
    float3 res = float3(8.0, 8.0, 8.0);
...
void DynamicVoronoi3D_half(half3 Coordinates, half Time, half AngleOffset, half CellDensity, out half Out, out half Cells)
{
    DynamicVoronoi3D_float(Coordinates, Time, AngleOffset, CellDensity, Out, Cells); 
}