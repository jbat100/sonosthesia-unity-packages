...
namespace Sonosthesia.Builder
{
    public static partial class Noise 
    {
        ...
        public struct Simplex1D<G> : INoise, ISimpleNoise where G : struct, IGradient, ISimpleGradient {
            ...
            public float4 GetNoiseValue4(float4x3 positions, SmallXXHash4 hash, int frequency)
            {
                positions *= frequency;
                int4 x0 = (int4)floor(positions.c0), x1 = x0 + 1;
                return default(G).EvaluateCombinedValue(
                    KernelValue(hash.Eat(x0), x0, positions) +
                    KernelValue(hash.Eat(x1), x1, positions)
                );
            }
            
            private static float4 KernelValue (SmallXXHash4 hash, float4 lx, float4x3 positions) 
            {
                float4 x = positions.c0 - lx;
                float4 f = 1f - x * x;
                float4 g = default(G).EvaluateValue(hash, x);
                return f * g * f * f;
            }
        }

        public struct Simplex2D<G> : INoise, ISimpleNoise where G : struct, IGradient, ISimpleGradient {
            ...
            public float4 GetNoiseValue4(float4x3 positions, SmallXXHash4 hash, int frequency)
            {
                ...
                return default(G).EvaluateCombinedValue(
                    KernelValue(h0.Eat(z0), x0, z0, positions) +
                    KernelValue(h1.Eat(z1), x1, z1, positions) +
                    KernelValue(hC.Eat(zC), xC, zC, positions)
                );
            }
            
            private static float4 KernelValue (SmallXXHash4 hash, float4 lx, float4 lz, float4x3 positions) 
            {
                float4 unskew = (lx + lz) * ((3f - sqrt(3f)) / 6f);
                float4 x = positions.c0 - lx + unskew, z = positions.c2 - lz + unskew;
                float4 f = 0.5f - x * x - z * z;
                float4 g = default(G).EvaluateValue(hash, x, z);
                return f * g * f * f * select(0f, 8f, f >= 0f);
            }
        }

        public struct Simplex3D<G> : INoise, ISimpleNoise where G : struct, IGradient, ISimpleGradient {
            ...
            public float4 GetNoiseValue4(float4x3 positions, SmallXXHash4 hash, int frequency)
            {
                ...
                return default(G).EvaluateCombinedValue(
                    KernelValue(h0.Eat(y0).Eat(z0), x0, y0, z0, positions) +
                    KernelValue(h1.Eat(y1).Eat(z1), x1, y1, z1, positions) +
                    KernelValue(hA.Eat(yCA).Eat(zCA), xCA, yCA, zCA, positions) +
                    KernelValue(hB.Eat(yCB).Eat(zCB), xCB, yCB, zCB, positions)
                );
            }
            
            static float4 KernelValue (SmallXXHash4 hash, float4 lx, float4 ly, float4 lz, float4x3 positions) 
            {
                float4 unskew = (lx + ly + lz) * (1f / 6f);
                float4
                    x = positions.c0 - lx + unskew,
                    ...
                    z = positions.c2 - lz + unskew;
                float4 f = 0.5f - x * x - y * y - z * z;
                float4 g = default(G).EvaluateValue(hash, x, y, z);
                return f * g * f * f * select(0f, 8f, f >= 0f);
            }
        }
    }
